...
class BaseModel {
    ...
    async removeFromSupabase(itemId, itemValue) {
        try {
            ...
            if (typeof showSuccessToast === "function") {
                showSuccessToast(
                    "Sucesso",
                    `${this.entityName} removido com sucesso`
                );
            }
        } catch (err) {
    ...
    clearTable() {
        ...
        if (tbody) {
            ...
            console.log(
                `🧹 ${this.entityName}: Tabela limpa (preservando thead)`
            );
        }
    }
    ...
    validateData(data) {
        ...
        Object.entries(this.validationRules).forEach(([field, rules]) => {
            ...
            if (rules.type === "number" && value && isNaN(parseFloat(value))) {
                errors.push(
                    `${rules.label || field} deve ser um número válido`
                );
            }

            if (rules.min && parseFloat(value) < rules.min) {
                errors.push(
                    `${rules.label || field} deve ser maior que ${rules.min}`
                );
            }
        });
