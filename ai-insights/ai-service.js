...
class GeminiAIService {
  constructor() {
    this.apiKey = "AIzaSyCTKSnwx4zLMq9odigyLVjqpojBkG_evdA";
...
RESPOSTA:`;
  ...
  formatFinancialContext(context) {
    ...
    if (
      ...
      summary.push(`- Receitas: ${this.formatCurrency(context.totalReceitas)}`);
      summary.push(`- Despesas: ${this.formatCurrency(context.totalDespesas)}`);
    ...
    if (
      ...
      Object.entries(context.categoriasDespesas)
        ...
        .forEach(([categoria, valor]) => {
          ...
          summary.push(`- ${categoria}: ${this.formatCurrency(valorSeguro)}`);
        });
      summary.push("");
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          console.error("❌ Gemini API Error Response (Texto):", errorMessage);
        }
