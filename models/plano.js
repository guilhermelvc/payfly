...
function formatarData(date) {
    try {
        ...
        console.log(
            `📅 formatarData - Input: "${date}" → Output: "${formatted}"`
        );
...
async function savePlano(descricao, valor, data, categoria = null) {
    try {
        ...
        if (isRecorrente && recorrenciaMeses > 1) {
            ...
            console.log(
                `✅ ${recorrenciaMeses} planos recorrentes criados com sucesso!`
            );
        } else {
            ...
            console.log("✅ Plano único criado com sucesso!");
        }
        ...
        showErrorToast(
            "Erro ao salvar",
            "Falha ao salvar plano. Tente novamente."
        );
    }
}
...
async function filterPlanos(event) {
    ...
    try {
        ...
        if (dataInput) {
            console.log(
                "🔍 DEBUG: Buscando todos os planos para comparar datas..."
            );
            ...
            allPlanos?.forEach((plano, idx) => {
                console.log(
                    `  ${idx + 1}. ID: ${plano.id.substring(
                        0,
                        8
                    )} | Descrição: "${plano.descricao}" | Data: "${
                        plano.data
                    }" (tipo: ${typeof plano.data})`
                );
            });
            console.log(
            ...
            if (rows && rows.length > 0) {
                ...
                rows.forEach((plano, idx) => {
                    console.log(
                        `  ${idx + 1}. DB: "${
                            plano.data
                        }" | Filtro: "${dataInput}" | Match: ${
                            plano.data === dataInput
                        }`
                    );
                });
            } else {
                console.log(
                    "❌ Nenhum resultado encontrado para a data:",
                    dataInput
                );
                const { data: allPlanos } = await window.supabase
                    .from("planos")
