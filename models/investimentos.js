...
async function saveInvestimento(
    ...
    try {
        ...
        if (isRecorrente && recorrenciaMeses > 1) {
            ...
            console.log(
                `✅ ${recorrenciaMeses} investimentos recorrentes criados com sucesso!`
            );
        } else {
        ...
        await reloadInvestimentoDataRespectingFilter();
        showSuccessToast(
            "Sucesso!",
            "Investimento(s) adicionado(s) com sucesso!"
        );
        Modal.close();
...
async function applyStoredInvestmentFilter() {
    ...
    try {
        const { descricao, valor, data, tipo } =
            currentInvestmentFilterCriteria;
        ...
        investments =
            rows?.map((item) => ({
                ...
                currentValue: parseFloat(
                    item.valor_atual || item.valor_investido
                ),
...
async function loadInvestmentsFromSupabase() {
    try {
        ...
        console.log(
            "📊 Total de investimentos encontrados:",
            data?.length || 0
        );
...
function updateTotalDisplay() {
    ...
    const rentabilidadeElement = document.getElementById(
        "rentabilidadeDisplay"
    );
    ...
    console.log(
        `📈 Totais atualizados: Investido: ${formatCurrency(
            totalInvestido
        )} | Atual: ${formatCurrency(valorAtual)} | Lucro: ${formatCurrency(
            lucro
        )}`
    );
}
...
function editInvestment(index) {
    ...
    if (realIndex === -1) {
        showErrorToast(
            "Erro",
            "Investimento não encontrado no array principal"
        );
        return;
    }
